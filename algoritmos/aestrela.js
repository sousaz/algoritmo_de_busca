...
function aEstrela(inicio, objetivo) {
    let aberto = [inicio];
    let fechado = [];
    ...
    let custoEstimado = new Map(); // f(n) = g(n) + h(n)
    custoEstimado.set(inicio, heuristica(inicio, objetivo));

    let caminho = new Map();

    while (aberto.length > 0) {
        // Encontrar o vértice em 'aberto' com o menor custoEstimado
        ...
        for (let adj of atual.adjacentes) {
            ...
            } else if (custoRealAcumulado >= custoReal.get(adj.vertice)) {
                continue;
            }

            // Este caminho é o melhor até agora, salvar
            caminho.set(adj.vertice, atual);
            custoReal.set(adj.vertice, custoRealAcumulado);
            custoEstimado.set(
                adj.vertice,
                custoReal.get(adj.vertice) + heuristica(adj.vertice, objetivo)
            );
        }
    }
